...
public class DetectEvent implements Listener {
    ...
    private void Detect(PlayerInteractEvent event) {
        ...
        if (player.isSneaking() && (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) && player.getInventory().getItemInMainHand().getType() == Material.COMPASS && event.getHand() == EquipmentSlot.HAND) {
            ...
            if (cache.get(String.format("OccSkillCoolDown.%s.Artisan_Detect", playerName)) != null) {
                MessageManager.ActionBarMessage(player, String.format("§c§l地质勘测§r§e冷却尚未结束  §e剩余 §b%s §e秒", cache.getInt(String.format("OccSkillCoolDown.%s.Artisan_Detect", playerName))));
                return;
            }
            ...
            cache.set(String.format("OccSkillCoolDown.%s.Artisan_Detect", playerName), coolDown);
